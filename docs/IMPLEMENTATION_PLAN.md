...
|---|---|---|
| **P0** | Restructure backend into `app/`, config, envelope, exception handlers, Alembic init, create the `tripzyy` database | **done** -- migration clean, `/health` 200 |
| **P1** | All models, first migration, seed script | **done** -- 13 tables, 42 CHECK constraints, 40 destinations, 212 activities, idempotent |
| **P2** | Auth: register / login / logout / me, JWT, revocation, password rules; users and preferences | **done** -- 43 tests |
| **P3** | Trips CRUD, status computation, ownership guard, pagination / filter / sort | **done** -- 43 tests |
| **P4** | Destination and activity search; stops; itinerary activities; reorder; transport; accommodation | **done** -- 60 tests |
| **P5** | Budget, expenses, calendar | **done** -- 39 tests |
| **P6** | Share, public trip, community listing, clone | **done** -- 23 tests |
| **P7** | Admin dashboard, analytics, user status | **done** -- 42 tests |
...
**Backend complete: 250 tests passing, 68 endpoints, 13 tables, no migration
drift.** `backend/scripts/e2e_check.py` walks section 40's Definition of Done
against a live server: 45 checks, all passing.
